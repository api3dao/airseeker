...
describe(updateFeedsLoopsModule.runUpdateFeeds.name, () => {
  ...
  it('fetches and processes other batches in a staggered way and logs errors', async () => {
    ...
    expect(logger.info).toHaveBeenNthCalledWith(1, 'Started update feeds loop.', expect.anything());
    ...
    expect(logger.info).toHaveBeenNthCalledWith(4, 'Finished update feeds loop.', {
...
describe(updateFeedsLoopsModule.processBatch.name, () => {
  ...
  it('does not scale gas price for the original (first) update transaction', async () => {
    ...
    expect(logger.debug).toHaveBeenCalledTimes(4);
    expect(logger.debug).toHaveBeenNthCalledWith(1, 'Processing batch of active data feeds.', expect.anything());
