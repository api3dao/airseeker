...
import { buildApiCalls, generateTestConfig, setAxios } from './data-fetcher';
...
describe('data fetcher', () => {
  ...
  it('tests the api call builder', async () => {
    const testConfig = generateTestConfig();

    const calls = buildApiCalls(testConfig, localDataStore);
    expect(calls.length).toEqual(9);

    // no duplicates
    expect(Object.keys(groupBy(calls, 'whoAmI')).length).toEqual(9);

    const deferredMockedFunction = jest.fn();
    ...
    const promisedCall = calls![1]!.fn(deferredMockedFunction);
    await expect(promisedCall).resolves.toBeUndefined();

    expect(deferredMockedFunction).toHaveBeenCalled();

    expect(mockAxios).toHaveBeenCalledWith({
      method: 'get',
      timeout: 9500,
      url: 'https://pool.nodary.io/0xC04575A2773Da9Cd23853A69694e02111b2c4182',
      headers: { Accept: 'application/json' },
    });

    const sample = await localDataStore.getStoreDataPoint(
      '0xC04575A2773Da9Cd23853A69694e02111b2c4182',
      '0x96504241fb9ae9a5941f97c9561dcfcd7cee77ee9486a58c8e78551c1268ddec'
    );
    expect(sample?.timestamp).toEqual('1687850583');
  });
});
